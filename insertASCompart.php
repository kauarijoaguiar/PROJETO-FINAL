...
if (isset($_GET["CODIGO"]) && !isset($_POST["Alterar"])) {
        ...
        $reacoes = $db->query("SELECT * FROM COMPARTILHAMENTO WHERE CODIGO = " . $_GET["CODIGO"] . "");
...
echo '<tr>';
echo '<td>';
echo '<h1>Incluir assuntos ao compartilhamento '.$_GET["CODIGO"].'</h1>';
echo '</td>';
echo '</tr>';
...
echo '</form>';
        ...
        if ($error == "") {
            $db = new SQLite3("face.db");
            $db->exec("PRAGMA foreign_keys = ON");
            $db->exec("insert into ASSUNTOCOMPART (CODIGOASSUNTO, ASSUNTOCOMPART) values ('" . $_POST["assunto"] . "', '" . $_POST["CODIGO"] ."')");
            $db->close();
