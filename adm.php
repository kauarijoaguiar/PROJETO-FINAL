...
primeiro = echo "UPDATE USUARIO SET ATIVO = 0 FROM (SELECT CASE WHEN MAX(DATAPOST) IS NULL THEN DATETIME('1900-01-01') ELSE MAX(DATAPOST) END AS DATAMAXIMA, EMAIL FROM USUARIO LEFT JOIN POST ON POST.EMAIL_USUARIO = USUARIO.EMAIL GROUP BY EMAIL) AS POST WHERE (USUARIO.EMAIL = POST.EMAIL AND POST.DATAMAXIMA < DATE(/*aaaaaaaa*/))) AND USUARIO.PAIS= $_POST['pais']";
